...
{this.markupArgs=[];var topBlock=[];var topOuts=[];var blocks=[];var info={args:this.markupArgs,argIndex:0};this.generateMarkup(topBlock,topOuts,blocks,info);this.addCode(topBlock,topOuts,blocks);var fnBlock=['(function (__code__, __context__, __in__, __out__'];for(var i=0;i<info.argIndex;++i)
...
{var name=child.parts[j].name;var names=name.split(".");args.push(names[0]);}}}}},generateMarkup:function(topBlock,topOuts,blocks,info)
{topBlock.push(',"<',this.tagName,'"');var name;for(name in this.attrs)
{if(name!=="class")
...
addParts(this.attrs["class"],',',topBlock,info,true);topBlock.push(', " "');for(name in this.classes)
...
{var path=[];var blocks=[];this.domArgs=[];path.embedIndex=0;path.loopIndex=0;path.staticIndex=0;path.renderIndex=0;var nodeCount=this.generateDOM(path,blocks,this.domArgs);var fnBlock=['(function (root, context, o'];var i;for(i=0;i<path.staticIndex;++i)
fnBlock.push(', ','s'+i);for(i=0;i<path.renderIndex;++i)
...
this.generateNodePath(path,blocks);var val;var arg;if(this.listeners)
{for(var i=0;i<this.listeners.length;i+=2)
{val=this.listeners[i+1];arg=generateArg(val,path,args);blocks.push('node.addEventListener("',this.listeners[i],'", __bind__(this, ',arg,'), false);\n');}}
if(this.props)
{for(var name in this.props)
{val=this.props[name];arg=generateArg(val,path,args);blocks.push("__prop__(node, '"+name+"', "+arg+");\n");}}
...
nodeCount+='+1';}
path[path.length-1]=basePath+'+'+loopName;blocks.push(loopName,' = __loop__.apply(this, [',iterName,', function(',counterName,',',loopName);for(var j=0;j<path.renderIndex;++j)
blocks.push(',d'+j);blocks.push(') {\n');blocks.push(subBlocks.join(""));blocks.push('return ',nodeCount,';\n');blocks.push('}]);\n');path.renderIndex=preIndex;return loopName;}});function Variable(name,format)
...
{this.tag.compile();var outputs=[];var html=this.renderHTML(args,outputs,self);var doc=before.ownerDocument;var tableParent=doc.createElement("div");tableParent.innerHTML="<table>"+html+"</table>";var tbody=tableParent.firstChild.firstChild;var parent=before.tagName.toLowerCase()==="tr"?before.parentNode:before;var after=before.tagName.toLowerCase()==="tr"?before.nextSibling:null;var firstRow=tbody.firstChild;var lastRow;while(tbody.firstChild)
